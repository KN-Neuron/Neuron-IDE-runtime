...
class CustomComponent : public Component {
   public:
    CustomComponent(std::shared_ptr<SceneObject> owner, 
                    std::shared_ptr<std::atomic<int>> updates, 
                    std::shared_ptr<std::atomic<int>> renders,
                    std::shared_ptr<std::stop_source> stopSource)
        : Component(std::move(owner)), updates(std::move(updates)), renders(std::move(renders)), stopSource(std::move(stopSource)) {}
...
class MarkerComponent : public Component {
   public:
    MarkerComponent(std::shared_ptr<SceneObject> owner, std::shared_ptr<std::stop_source> stopSource)
...
TEST(RendererTest, RenderLoop_WhenComponentAdded_CallsUpdateExactlyOnceBeforeStop) {
    ...
    SDL_Surface* surface = SDL_CreateRGBSurfaceWithFormat(0, 10, 10, 32, SDL_PIXELFORMAT_RGBA32);
...
TEST(RendererTest, RenderLoop_QueuesMarkersFromComponents) {
    ...
    if (dequeued) {
        ...
        // Ensure no other markers are left
